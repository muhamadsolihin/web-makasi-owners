  ...
  setup() {
    ...
    const submit = (): void => {
      ...
      if (active.value == 0) {
        formData.append(
          "phone",
          phone.value != undefined ? `62${phone.value}` : ""
        );
        Login.SET_USER_TYPE_LOGIN(0);
        RegistrationState.SET_AUTH_USE(1);
      } else {
        formData.append("email", email.value != undefined ? email.value : "");
        Login.SET_USER_TYPE_LOGIN(1);
        RegistrationState.SET_AUTH_USE(2);
      }

      Login.SET_LOADING(true);
      if (typeLogin.value) {
        // Handling email auth
        Login.checkAccountAvailable(formData).then((res) => {
          if (res.status) {
            if (res.data.user_type_name == "Owner") {
              switch (res.data.code) {
                case "REG-E/I":
                  ...
                  RegistrationState.SET_DATA_REGISTRATION_FORM({
                    email: email.value,
                    phone: "",
                  });
                  RegistrationState.SET_REGISTRATION_STATUS(true);
          ...
          if (res.status) {
            if (res.data.user_type_name == "Owner") {
              Login.SET_USER_STATE({
                ...
                // eslint-disable-next-line
                token_fcm: window.sessionStorage.getItem("token_fcm") as string,
              });
              router.push("/login/enter-pin");
            } else {
              ElNotification({
